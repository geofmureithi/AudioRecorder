...
fn main() {
    ...
    let in_src = "src/audio_glue.rs.in";
    let out_src = Path::new(&out_dir).join("audio_glue.rs");
    ...
    let java_folder = Path::new("../app/src/main/java/com/example/audiorecorder/lib");
    if java_folder.exists() {
        fs::remove_dir_all(java_folder);
    }
    fs::create_dir(java_folder).unwrap();
    let swig_gen = flapigen::Generator::new(LanguageConfig::JavaConfig(
        JavaConfig::new(java_folder.into(), "com.example.audiorecorder.lib".into())
